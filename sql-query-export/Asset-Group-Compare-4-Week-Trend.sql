...
WITH
   timestamps AS (
      SELECT date (ts) AS upper_date, date (ts - INTERVAL '1 weeks') AS lower_date
      FROM generate_series(now() - INTERVAL '3 weeks', now(), INTERVAL '1 week') AS ts
   ),
   asset_scans AS (
 ...
 SELECT ds.name, sctf.*, scvs.total_vulnerabilities, scvs.moderate_vulns, scvs.severe_vulns, scvs.critical_vulns,
CASE WHEN sctf.date = date(now()) THEN 'Week 4'
...
END AS week
   FROM site_change_totals_flattened sctf
