...
SELECT
  ...
  CASE
    WHEN
      s.accesslevel IS NOT NULL 
    THEN
      s.accesslevel 
    ELSE
      dacs.aggregated_credential_status_description 
  END
  AS "access level" , 0 AS "exclusions" , 
  CASE
    WHEN
      ds.name = 'Rapid7 Insight Agents' 
    THEN
      1 
    ELSE
      0 
  END
  AS "agent" , porttext AS "ports" 
FROM
  dim_asset da 
  JOIN
    fact_asset fa 
  ...
  JOIN
    dim_aggregated_credential_status dacs 
  ...
  LEFT JOIN
    (
      ...
      FROM
        ...
        JOIN
          fact_asset fa1 
          ON da1.asset_id = fa1.asset_id 
        JOIN
          dim_asset_service_credential dsc1 
          ON da1.asset_id = dsc1.asset_id 
        JOIN
          dim_aggregated_credential_status dacs1 
          ON dacs1.aggregated_credential_status_id = fa1.aggregated_credential_status_id 
          AND dacs1.aggregated_credential_status_description = 'All credentials successful' 
      GROUP BY
        da1.asset_id 
    )
    sub 
    ON da.asset_id = sub.asset_id 
  LEFT JOIN
    (
      SELECT
        asset_id,
        ...
        CASE
          WHEN
            SUM( "authentication partial success") = 12 
          THEN
        ...
        (
          SELECT DISTINCT
            ...
            CASE
              WHEN
                dsc.port = 139 
                AND dcs.credential_status_description = 'Login successful' 
              THEN
                5 
              WHEN
                dsc.port = 445 
              ...
              WHEN
                dsc.port = 139 
                AND dcs.credential_status_description = 'Login failed' 
              THEN
                1 
              WHEN
                dsc.port = 445 
              ...
              ELSE
                0 
            END
            AS "authentication partial success" 
          FROM
            dim_asset da 
            JOIN
              dim_asset_service_credential dsc 
              ON da.asset_id = dsc.asset_id 
            JOIN
              dim_credential_status dcs USING (credential_status_id) 
          WHERE
