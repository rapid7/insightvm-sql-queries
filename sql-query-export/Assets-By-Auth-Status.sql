...
WITH 
max_certainty AS (SELECT asset_id, max(certainty) AS certainty
FROM dim_asset_operating_system  
GROUP BY asset_id
),
asset_cred_status AS (SELECT DISTINCT fa.asset_id,
...
SELECT acs.asset_id, da.ip_address, da.host_name, acs.auth_status,
