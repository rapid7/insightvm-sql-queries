...
WITH asset_data as (SELECT da.asset_group_id as group_id, count(da.asset_id) as assets, sum(fa.vulnerabilities) as vulns, ROUND(AVG(fa.riskscore)) as avg_host_score, ROUND(MAX(fa.riskscore)) as highest_host
                        ...
                        GROUP BY da.asset_group_id),

score_rank as (SELECT ad.group_id as group_id, CASE WHEN ad.avg_host_score > 35000 THEN 5
...
host_curve as (SELECT ad.group_id as group_id, CASE WHEN ad.highest_host > 75000 THEN sr.score+4
...
SELECT dag.name as group_name, ad.assets, ad.vulns, ad.avg_host_score, ad.highest_host,
